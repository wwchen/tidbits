...
class BinaryCode:
  ...
  def decode(self, message):
    def getSuffix(prev, curr, msg):
      # get the next number.. assuming msg isn't empty
      nxt = int(msg[0]) - curr - prev
      if nxt < 0 or nxt > 1:
        return "NONE" 

      # case when you are the last digit
      if len(msg) < 2:
        return str(curr) if nxt == 0 else "NONE"

      suffix = getSuffix(curr, nxt, msg[1:])
      if 'NONE' in suffix:
        return "NONE"
      return str(curr) + suffix

    zero = getSuffix(0, 0, message)
    one  = getSuffix(0, 1, message)
    print  zero, one
    return zero, one
